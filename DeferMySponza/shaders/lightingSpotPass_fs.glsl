...
layout(std140, binding=4) uniform SpotLightBuffer{
	SpotLight lights[5];
};
...
void main()
{
	...
	float NoV = saturate(dot(N, V)) + 0.01f;
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		float inverseSquareRadius = 1.0 / (lights[i].range * lights[i].range);
		float lightAngleScale = 1.0f / max(0.001f, ((lights[i].coneAngle /2.0) - lights[i].coneAngle));
		float attenuation = 1.0;
		attenuation *= getDistanceAtt(lights[i].position - FragPos, inverseSquareRadius);
		attenuation *= getAngleAtt(L, lights[i].direction, lightAngleScale, 2.0);
		...
		vec3 light_colour = lights[i].intensity * attenuation;
